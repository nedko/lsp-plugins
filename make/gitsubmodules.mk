...
$(ALL_SRC_MODULES) $(ALL_HDR_MODULES) $(ALL_BIN_MODULES) $(ALL_PLUG_MODULES):
	printf "% 30s: " "$($(@)_NAME)"
	if (git submodule status "$($(@)_PATH)" >/dev/null 2>&1); then echo ok; else git submodule add "$($(@)_URL)" "modules/$($(@)_NAME)"; fi

gitsubmodules: $(SRC_MODULES) $(HDR_MODULES) $(BIN_MODULES) $(PLUG_MODULES)
